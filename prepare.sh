...
srun --ntasks-per-node=1 bash -c '
  mkdir -p /scratch/cache/cambrian_processed && \
  cd /fsx/luis_wiedmann/data-dedup/data/cambrian_processed && \
  find . -type f | parallel -j 16 rsync -R {} /scratch/cache/cambrian_processed/
'
