...
srun --ntasks-per-node=1 bash -c '
  mkdir -p /scratch/cache/asterix_deduplicated && \
  cd /fsx/luis_wiedmann/data-dedup/data/asterix_deduplicated && \
  find . -type f | parallel -j 16 rsync -R {} /scratch/cache/asterix_deduplicated/
'
